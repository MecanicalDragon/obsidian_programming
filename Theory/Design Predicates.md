Design Predicates is a method to quantify the complexity of the integration between the two units of code. Every design predicate has associated *integration complexity rating*. For the code units that imply more than one design predicate, integration complexity ratings can be combined. A number of tests required to cover all test cases equals total complexity + 1 and the [[Code Complexity#Cyclomatic complexity|cyclomatic complexity]] of the code. Design predicates are useful in [[Test Types#^bpt|basis path testing]].
