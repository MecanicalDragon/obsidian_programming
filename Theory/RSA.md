...
`14^27` - сложное вычисление, но с помощью модульной арифметики и представления 27 степенями двойки это получается быстро и эффективно.

Для начала представляем 27 как сумму степеней двоек.
`27 = 16 + 8 + 2 + 1 = 2^4 + 2^3 + 2^1 + 2^0`

Теперь считаем остаток от деления `enc` в каждой степени двойки до максимальной используемой в `27` (`d`). При этом на каждом следующем этапе мы подставляем в выражение уже вычисленный ранее результат.
- `14^1 %  55 = 14`
...
Одно из свойств модульной арифметики - *согласованность операций в кольце вычетов*, то есть, `(n mod m)^d mod m = n^d mod m`. Таким образом, если `n` - исходное сообщение, `enc = n^e mod m` => `dec = enc^d mod m = (n^e mod m)^d mod m = n^ed mod m`, но `ed ≡ 1 mod φ(m)` - мы так их сами выбрали.
