...
-- RANK skips numbers after same places  
...
select name,  
      score,  
      row_number() over w as "order",  
      rank() over w as "prize place",  
      dense_rank() over w as "dense prize place"  
...
      gender,  
      score,  
      coalesce(lag(name) over (partition by gender order by score asc), 'FIRST') as "eliminate after"  
from skiers;  
...
select name,  
      score,  
      row_number() over (partition by discipline order by score desc) as "order",  
      rank() over (partition by discipline order by score desc) as "prize place",  
      dense_rank() over (partition by discipline order by score desc) as "dense prize place"  
from athlete;  
...
-- Результат работы оконной функции невозможно отфильтровать в запросе с помощью WHERE, потому что оконные функции  
